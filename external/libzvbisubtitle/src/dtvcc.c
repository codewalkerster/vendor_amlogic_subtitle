	...
	case DIR_BOTTOM_TOP:
		...
		else
		{
			...
			if (row + 1 < dw->row_count) {
				...
				// carriage return + print[left->rignth] + scroll bottom->top = roll up
				 ...
				 uint8_t	delay_time)
{
	...
	{
		/* Set trigger time
			Until that time, service stop decoding.
			*/
...
/* valid_byte_pair: 0xFE start header
	return value: valid_byte_pair + 1(0xFF 3 bytes)
	*/
static char
calc_packet_code_byte(struct dtvcc_decoder *	dc, const uint8_t * buf, int cc_count)
{
	uint8_t valid_byte_pair = 0;
	...
	packet_code_byte = (valid_byte_pair + 1);
...
calc_block_size_byte(const uint8_t * buf, int cc_count)
{
	...
	int valid_byte_pair = 0;
	for (int i = 0; i < cc_count; ++i) {
		if (buf[3 + i * 3] == 0xfe) {
			valid_byte_pair++;
		}
	}
	valid_data_bytes = valid_byte_pair * 2;
